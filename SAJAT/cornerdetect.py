...
# 1. Kép betöltése és szürkítése
img = cv2.imread('../sakktabla.jpg')
gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

# 2. Előfeldolgozás (enyhe blur)
gray = cv2.GaussianBlur(gray, (5, 5), 0)

# 3. Pontok detektálása (Shi-Tomasi)
# maxCorners=500, qualityLevel=0.05, minDistance=30
corners = cv2.goodFeaturesToTrack(gray, 500, 0.1, 30)

# 4. Koordináták kinyerése (NumPy tömbbé alakítás)
if corners is not None:
    points = corners.reshape(-1, 2).astype(np.float32)
    print(f"Talált pontok száma: {len(points)}")


    # 5. Pontok kirajzolása az eredeti képre
    for x, y in points:
...
H_matrix, mask = cv2.findHomography(image_pts, logical_offset_pts, cv2.RANSAC, 5.0)

# 3. Kép transzformálása (Warping)
warped_img = cv2.warpPerspective(img, H_matrix, (dest_size, dest_size))
