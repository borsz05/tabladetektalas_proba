...
if ids is not None:
    ...
    if len(ids) == 4:
        ...
        try:
            ...
            cv.imshow("Felulnezet, warped", warped)
...
if len(points) > 10:
    ...
    if vh is not None and vv is not None:
        ...
        for pt in projected_pts:
            px, py = int(pt[0]), int(pt[1])
            cv.circle(annotated_img, (px, py), 10, (255, 255, 255), -1) # Fehér alap
            cv.circle(annotated_img, (px, py), 10, (0, 255, 0), -1)   # Piros pont (rekonstruált)
